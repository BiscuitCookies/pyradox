...
def get_localisation(key, game, process_substitutions = True):
    ...
    if game not in localisation_cache or 'HoI4mod' not in localisation_cache:
        localisation_path = os.path.join(pyradox.get_game_directory(game), 'localisation')
        localisation_pathmod = os.path.join(pyradox.get_game_directory('HoI4mod'), 'localisation')
        localisation_cache[game] = parse_walk(localisation_path)
        localisation_cache['HoI4mod'] = parse_walk(localisation_pathmod)
    ...
    if key.lower() in localisation_cache[game]:
        ...
        return result
    elif key.lower() in localisation_cache['HoI4mod']:
        result = pyradox.token.make_string(localisation_cache['HoI4mod'][key.lower()])
        if process_substitutions:
            # TODO: are $ signs escapable?
            result = re.sub(r'\$(.*?)\$', replace_substitution, result)
        return result
